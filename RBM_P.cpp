...
//                    int i = LS->ids[m];
            ...
            update_w(w_acc, w_count, (epoch - 1) * N + batch);
            update_vb(vb_acc, vb_count, (epoch - 1) * N + batch);
            update_hb(hb_acc, (epoch - 1) * N + batch);
...
//        }
    ...
    for(int dd = 0; dd < M * F; dd++)
        printf("after train_full: w[%d]: %lf\n", dd, w[dd]);
    for(int dd = 0; dd < M * F; dd++)
        printf("after train_full: d[%d]: %lf\n", dd, d[dd]);
